...
void feistel_encode(uint32_t *N1, uint32_t *N2, uint64_t *keys48b);

void feistel_decode(uint32_t *N1, uint32_t *N2, uint64_t *keys48b);
...
void substitution_6bits_to_4bits(uint8_t *blocks6b, uint8_t *blocks4b) {
    ...
    for (uint8_t i = 0, j = 0; i < 8; i += 2, ++j) {
        ...
        blocks4b[j] = s_box[i][block2b][block4b];

        block2b = extreme_bits(blocks6b[i + 1]);
        block4b = middle_bits(blocks6b[i + 1]);
        blocks4b[j] = (blocks4b[j] << 4) | s_box[i + 1][block2b][block4b];
    }
}
